...
router.post('/list', async (req, res) => {
  try {
    ...
    if (rank === 'dono' || rank === 'sub') {
      registros = await query<any[]>('SELECT * FROM chas ORDER BY created_at DESC LIMIT 1000');
    } else {
      registros = await query<any[]>('SELECT * FROM chas WHERE admin_id = ? ORDER BY created_at DESC LIMIT 1000', [admin_id]);
    }
